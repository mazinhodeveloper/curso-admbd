...
-- Apaga a procedure antiga, se ela existir, para evitar erros
...
BEGIN
    -- =================================================================================
    -- PARTE 1: Saldo Total Consolidado do Usuário
    -- Esta parte utiliza a sua Function, que está correta e foi mantida.
    -- =================================================================================
    SELECT 
        fn_saldo_usuario(p_id_usuario) AS saldo_total_consolidado;
 
 
    -- =================================================================================
    -- PARTE 2: Resumo de GASTOS por Categoria (LÓGICA CORRIGIDA)
    -- CORREÇÃO: Agora, a consulta busca o usuário tanto da conta (para débitos)
    -- quanto do cartão (para despesas de fatura), usando COALESCE.
    -- MELHORIA: A consulta foca apenas em operações de SAÍDA, para um resumo de GASTOS.
    -- =================================================================================
    SELECT 
        cat.categoria_nome, 
        SUM(t.transacao_valor) AS total_gasto_categoria
    FROM 
        transacao AS t
    JOIN 
        categoria AS cat ON t.fk_id_categoria = cat.id_categoria
    JOIN 
        operacao AS o ON t.fk_id_operacao = o.id_operacao
    LEFT JOIN 
        conta AS c ON o.fk_id_conta = c.id_conta
    LEFT JOIN 
        fatura AS f ON o.fk_id_fatura = f.id_fatura
    LEFT JOIN 
        cartao AS ca ON f.fk_id_cartao = ca.id_cartao
    WHERE 
        -- A MÁGICA DO COALESCE: Pega o ID do usuário da primeira fonte que não for nula (conta ou cartão)
        COALESCE(c.fk_id_usuario, ca.fk_id_usuario) = p_id_usuario
        -- Filtro de data padrão
        AND t.transacao_data BETWEEN p_data_inicio AND p_data_fim
        -- Filtro para considerar apenas GASTOS (saídas)
        AND o.operacao_nome IN ('conta_saida', 'fatura_saida')
    GROUP BY 
        cat.categoria_nome
    ORDER BY
        total_gasto_categoria DESC;
 
 
    -- =================================================================================
    -- PARTE 3: Faturas Pendentes (Abertas ou Atrasadas)
    -- Esta parte já estava logicamente correta e foi mantida.
    -- =================================================================================
    SELECT 
        f.id_fatura, 
        f.fatura_data_inicio, 
        f.fatura_data_fim, 
        f.fatura_valor_total, 
        s.status_fatura_nome
    FROM 
        fatura AS f
    JOIN 
        cartao AS ca ON f.fk_id_cartao = ca.id_cartao
    JOIN 
        status_fatura AS s ON f.fk_id_status = s.id_status_fatura
    WHERE 
        ca.fk_id_usuario = p_id_usuario 
        AND s.status_fatura_nome IN ('aberta', 'atrasada');
END$$
