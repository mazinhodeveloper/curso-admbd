...
--
-- Host: 127.0.0.1
-- Tempo de geração: 28/07/2025 às 19:02
...
--
-- Estrutura stand-in para view `conta`
-- (Veja abaixo para a visão atual)
--
CREATE TABLE `conta` (
`descricao_centro_custos` varchar(50)
,`total` bigint(21)
);
...
--
-- Estrutura stand-in para view `custos`
-- (Veja abaixo para a visão atual)
--
CREATE TABLE `custos` (
`descricao_centro_custos` varchar(50)
...
--
-- Estrutura para view `conta`
--
DROP TABLE IF EXISTS `conta`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `conta`  AS SELECT `c`.`descricao_centro_custos` AS `descricao_centro_custos`, round(count(`p`.`valor`),2) AS `total` FROM (`pagamentos` `p` join `centro_custos` `c` on(`c`.`id_centro_custos` = `p`.`id_centro_custos`)) GROUP BY `c`.`descricao_centro_custos` ;
...
--
-- Estrutura para view `custos`
--
DROP TABLE IF EXISTS `custos`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `custos`  AS SELECT `c`.`descricao_centro_custos` AS `descricao_centro_custos`, round(sum(`p`.`valor`),2) AS `total` FROM (`pagamentos` `p` join `centro_custos` `c` on(`c`.`id_centro_custos` = `p`.`id_centro_custos`)) GROUP BY `c`.`descricao_centro_custos` ;
